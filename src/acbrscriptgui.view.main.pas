...
var
  ...
  sl: TStringList;
begin
  sl := FindAllFiles(edtPathACBr.Text, '*.lpk', True);
  try
    for i := 0 to sl.Count - 1 do
      outputScreen.Lines.Add(LowerCase(sl.Strings[i]));
  finally
    buttonValidatePathACBr.ImageIndex := ifthen(i >= 30, 4, 3);
    strListACBr := sl;
  end;
end;
...
begin
  if (buttonValidatePathACBr.ImageIndex = 3) or
    (buttonValidatePathLazarus.ImageIndex = 3) then
  begin
  ...
  try
    ...
    dialogBoxAutoClose('', 'ACBr FOI INSTALADO NO LAZARUS! ' + sLineBreak +
      TimeToStr(TimeFinish), 6);
